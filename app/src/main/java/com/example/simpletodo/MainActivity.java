...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        if (resultCode == RESULT_OK && requestCode == EDIT_TEXT_CODE) {
            ...
            // Get and extract the original position of the edited text item
            int position = data.getExtras().getInt(KEY_ITEM_POSITION);
            // Update to the new item text
            items.set(position, itemText);
            // Notify the adapter
            itemsAdapter.notifyItemChanged(position);
            // Save the new text in its position
            saveItems();
            Toast.makeText(getApplicationContext(), "Item has been updated successfully!", Toast.LENGTH_SHORT).show();
        } else {
            Log.w("MainActivit", "Unknown call to onActivityResult");
        }
    }
    ...
    // Function to load items by reading each line of data file
